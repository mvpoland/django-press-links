...
class Entry(models.Model):
    ...
    slug = models.SlugField(
        max_length=255, unique_for_date='pub_date', verbose_name='slug'
    )
    pub_date = models.DateTimeField(
        default=datetime.now, verbose_name=_('publication date')
    )
    status = models.IntegerField(
        choices=STATUS_CHOICES, default=DRAFT_STATUS, verbose_name=_('status')
    )
    excerpt = models.TextField(blank=True, verbose_name=_(u'Excerpt'))
    source = models.CharField(
        max_length=255, verbose_name=_('the source for the entry'), blank=True
    )
...
class Link(models.Model):
    ...
    link_text = models.CharField(
        max_length=255, verbose_name=_('text for link')
    )
    link_new_page = models.BooleanField(
        default=False, verbose_name=_('open link in new page')
    )
